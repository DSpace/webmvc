    ...
    <tr>
        <td align="left">
            <#if item??>
                ...
                <h1><@dspace.messageArgs "ui.dspace-admin.authorize-item-edit.policies", showingArgs /></h1>
            </#if>
        </td>
            ...
            <table class="miscTable" align="center" summary="Bundle Policy Edit Form">
                ...
                <#list myPolicies as rp>
                    <tr>
                        <td class="${row}RowOddCol">${rp.getID()}</td>
                        <td class="${row}RowEvenCol">${rp.getActionText()}</td>
                        <td class="${row}RowOddCol"><#if rp.getEPerson()??>${rp.getEPerson().getEmail()}</#if></td>
                        <td class="${row}RowEvenCol"><#if rp.getGroup()??>${rp.getGroup().getName()}</#if></td>
                        <td class="${row}RowOddCol">
                            <form method="post" action="<@dspace.url "/admin/authorize" />">
                                <input type="hidden" name="policy_id" value="${rp.getID()}" />
                                <input type="hidden" name="item_id" value="<#if item??>${item.getID()}</#if>" />
