...
<table width="95%">
    <tr>
        <td align="left">
            <#if community??>
                <#assign showingArgs=[community.getMetadata("name"), community.getHandle(), community.getID()]>
                <h1><@dspace.messageArgs "ui.dspace-admin.authorize-community-edit.policies", showingArgs /></h1>
            </#if>
        </td>
    ...
    <#if policies??>
        <#list policies as rp>
            <tr>
                ...
                <td headers="t2" class="${row}RowEvenCol">${rp.getActionText()}</td>
