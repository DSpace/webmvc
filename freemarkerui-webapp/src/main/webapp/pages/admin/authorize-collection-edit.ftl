...
<table width="95%">
    <tr>
        <td align="left">
            <#if collection??>
                <#assign showingArgs=[collection.getMetadata("name"), collection.getHandle(), collection.getID()]>
                <h1><@dspace.messageArgs "ui.dspace-admin.authorize-collection-edit.policies", showingArgs /></h1>
            </#if>
        </td>
        ...
        <form action="<@dspace.url "/admin/authorize" />" method="post">
            <table class="miscTable" align="center" summary="Collection Policy Edit Form">
                ...
                <tr>
                    ...
                    <td class="${row}RowOddCol"><#if rp.getGroup()??>${rp.getGroup().getName()}<#else>${"..."}</#if></td>
